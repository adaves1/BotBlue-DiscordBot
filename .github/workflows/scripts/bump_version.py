...
if int(os.environ.get("JUST_RETURN_VERSION", 0)):
    set_output("version", workabot._VERSION)
    sys.exit(0)
...
def repl(match: Match[str]) -> str:
    ...
    if new_stable_version == "auto":
        version_info = workabot.VersionInfo.from_str(match.group("version"))
        version_info.dev_release = None
    else:
        version_info = workabot.VersionInfo.from_str(new_stable_version)
...
with open("workabot/__init__.py", encoding="utf-8") as fp:
...
with open("workabot/__init__.py", "w", encoding="utf-8", newline="\n") as fp:
